			...
			<div className="flex flex-col items-center justify-center space-y-10 flex-1">
				<div className="bg-primary relative rounded-full mt-24">
					{['w-[36rem] h-[36rem] animate-mid-spin', 'w-96 h-96 animate-slow-spin'].map((size, i) => (
						<motion.div initial={{ opacity: 0 }} animate={{ opacity: .5 }} transition={{ delay: 0.2 * i }} key={i} className={size + " absolute opacity-50 top-1/2 left-1/2 -z-1 bg-red-500 -translate-1/2 bg-gradient-to-r from-orange-400 to-orange-400 via-primary rounded-full p-px"}>
